...
module masterModule {
    ...
    export var totalResult = (model: MasterModel) =>
    {
        ...
        TR(TH("Total games: "), TD({ style: "text-align:right;" }, result.totalGames)),
        TR(TH("Total spent: "), TD({ style: "text-align:right;" },result.totalSpent)),
        TR(TH("Max win: "), TD({ style: "text-align:right;" },result.maxWin)),
        TR(TH("Total win: "), TD({ style: "text-align:right;" },result.totalWin)),
        TR(TH("Percentage win: "), TD({ style: "text-align:right;" },Math.round(result.totalWin * 100 / result.totalSpent).toString() + "%"))
        );
    ...
    export var masterView = (model: MasterModel) =>
        DIV({class : "container"},
            ...
            DIV({ id: "rightContent", class:"span6" },
                ...
                H3("Total results"),
                TABLE({ class: "table table-bordered" },TR(TD({ "colspan": 2 }, BUTTON({ id : "addgame", style : "display:none;", class : "btn" }, "Add game")))),
